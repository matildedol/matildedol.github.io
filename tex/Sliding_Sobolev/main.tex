...
It is a \textbf{bound on the entropy} in terms of a norm of the gradient of our observable:
    ...
    \end{equation}
where $\mathcal{I}$ is the Fisher information $\mathcal{I}(\rho):= \int \rho | \nabla \log \rho |^2 \de \pi = \lVert \nabla \log \rho \rVert_\rho ^2$
\end{block}
\pause[]
\bigskip
...
{   \small
    ...
    Hence
    \begin{equation*}
        \mathcal{H}(P_t\rho) \le - \frac{1}{2\lambda} \frac{\de}{\de t}\mathcal{H}(P_t \rho); \quad \frac{\de}{\de t}\mathcal{H}(P_t \rho)\le -2\lambda \mathcal{H}(P_t\rho)
    \end{equation*}
...
%    \begin{center}Our focus: convergence to equilibrium in terms of the \textbf{mixing time} of the Markov chain
%    \end{center}

   \begin{block}{Total variation mixing time of a Markov chain}
    Let $P_t$ be the Markov semigroup on the space of densities $\mP(\X)$ and $P^*_t$ be its dual acting on probability measures.\\
    For $\varepsilon > 0$, 
    \[
    ...
    \end{block}
\pause
   {\color{blue} $\rightarrow$ Relation to entropy:}
    \begin{block}{Pinsker's inequality}
        Let $\mu$ be a probability distribution and $\pi$ be the uniform distribution over $\X$. Then
        \begin{equation*}\tag{Pinsker}
            \lVert \rho -\pi \rVert_{TV}^2 \le \frac{1}{2} \mathcal{H(\rho)}
        \end{equation*}
    \end{block}
...
\begin{frame}[allowframebreaks]{Additional formulas}
    ...
    \begin{block}{Induced Riemannian structure on $\X$}
        \begin{equation*}
            {(\Psi,\Phi)}_\rho:=\frac{1}{2}\sum_{x,y}\Psi(x,y)\Phi(x,y)\hat{\rho}(x,y)Q(x,y)\pi(x)
        \end{equation*}
    \end{block}
