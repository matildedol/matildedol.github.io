...
\pause
    \begin{block}{$\rightarrow$ Relation to entropy: Pinsker's inequality}
        Let $\mu, \pi$ be two probability distributions over $\X$. Then
        \begin{equation*}\tag{Pinsker}
            \lVert \mu -\pi \rVert_{TV}^2 \le \frac{1}{2} \mathcal{H(\mu | \pi)}
        \end{equation*}
    \end{block}
...
\\~\\
    ...
    \begin{block}{Convergence of the ZRP}
        ...
        \[
        \tau_{\rm mix}(\varepsilon) \le K^2 L \log L \bigg(\frac{1}{8} - \frac{\log \varepsilon}{c}\bigg)
        \]
        for some universal constant $c$
    \end{block}
\end{frame}  
