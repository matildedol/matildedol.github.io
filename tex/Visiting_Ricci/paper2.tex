...
Functional Inequalities for Markov Chains with Non-Negative Ricci Curvature}}

Reference: \cite{erbarfathi}, find it \href{https://www.sciencedirect.com/science/article/pii/S0022123618301101}{here}
...
\subsubsection{Getting convergence to equilibrium}

The MLSI estimate for the ZRP is then used to get converegence to equilibrium of the process. In particular, this convergence is expressed in terms of the \textbf{mixing time}. 
...
\begin{definition}[Total variation mixing time]
    The total variation mixing time is defined for $\varepsilon >0$ as
    \begin{equation*} 
...
\begin{theorem}[Bound on mixing time of ZRP]\label{thm: result}
...
\textcolor{blue}{EB: E’ necessario chiarire come tutti questi concetti e queste
disuguaglianze si combinano per ottenere la stima sul tempo di
mixing.}

\textcolor{blue}{(1) Cosa centra MLSI con il fatto che il semigruppo è il flusso
dell’entropia? Questo punto è talmente fondamentale che potresti
considerare di far vedere il calcolo per ottenere entropy decay}

\textcolor{blue}{(2) Capito (1), come si combina entropy decay e Pinsker per concludere? Anche questo è da esplicitare}
\\

(1) \textbf{How MLSI + $P_t$ being the heat semigroup + Fisher being the derivative of the entropy imply entorpy decay?}

[Continuous case]

That $P_t$ is the heat semigroup follows by construction of $\W$, and its relation to the gradient flow of the entropy was derived in Section \ref{sec: heat flow antropy}. 
We show how Fisher is the derivative of the entropy.

Recall that for a probability densty $\rho \in \mP(X)$,
\begin{align*}
    &\mathcal{H}(\rho) := \int \rho \log\rho \de \pi \\
    &\mathcal{I}(\rho) := \int \rho |\nabla \log \rho |^2 \de \pi
\end{align*}

Then, we have
\begin{align*}
    \frac{\de}{\de t} \mathcal{H}(P_t\rho) &= \int \frac{\de}{\de t} P_t\rho \log(P_t\rho)\de \pi \\
    &= \int \bigg(\frac{\de}{\de t} P_t \rho\bigg) \log P_t \rho + \frac{1}{P_t \rho} P_t \rho \bigg(\frac{\de}{\de t} P_t \rho\bigg) \de \pi \\
    &= \int \frac{\de}{\de t} P_t \rho\Big( \log P_t \rho + 1\Big) \de \pi = \int \Delta P_t \rho\Big(\log P_t \rho + 1\Big) \de \pi 
\end{align*}
where in the last passage $\Delta$ is the Laplacian operator and equality follows from $P_t$ being the heat semigroup. 
Then, integrating by parts and usnig that $\nabla \log f = \nabla f / f$ we get
\begin{align*}
    \frac{\de}{\de t} \mathcal{H}(P_t\rho) = \int \nabla P_t \rho \cdot \frac{\nabla P_t \rho}{P_t \rho} \de \pi = \mathcal{I}(P_t \rho)
\end{align*}

\begin{remark} Consider the generator of the chain $\mathcal{L}$, the invariant measure $\pi$ and functions $f:X \to \R$. 
For consistency with most of the literature, define the Dirichlet form as 
\begin{equation*}
\mathcal{E}(f,g) := \int_\X f \mathcal{L} g \de \pi
\end{equation*}
Then, we define Fisher infromation of a probability denisty $\rho \in \mathcal{P}(X)$ as
\begin{equation*}
\mathcal{I}(\rho)= \int_X \lVert \nabla \log\rho \rVert^2 \de \pi
\end{equation*}
...
\begin{equation}\tag{MLSI}
    \mathcal{H}(\rho) \le \frac{1}{2 \lambda} \mathcal{I}(\rho)
\end{equation}
by using the result above we get that the entropy is bounded by its derivative as 
\begin{equation*}
    \mathcal{H}(P_t\rho) \le -\frac{1}{2\lambda} \mathcal{H}'(P_t\rho)
\end{equation*}
Notice that this implies
\begin{equation*}
    \mathcal{H}'(P_t\rho) \le - 2 \lambda \mathcal{H}(P_t\rho)
\end{equation*}
...
Gronwall's inequality is a fundamental result in evolution problems. It states that a nonnegative function $\phi(t)$ such that there exists a constant $C$ such that $\phi'(t) \le C(t) \phi(t)$ satisfies 
\begin{equation*}
\phi(t) \le \phi(0) \exp \bigg(\int_0^t C(\tau) \de \tau\bigg)
\end{equation*}
\begin{proof}
Consider the quantity 
\begin{equation*}
    v(t) := \exp \bigg(\int_a^t C(\tau) \de \tau\bigg)
\end{equation*}
and observing that $v'(t)= v(t)C(t)$, $v(a)=1$. 
Then using the quotient formula we have
\begin{equation*}
    \frac{\de}{\de t}\frac{\phi(t)}{v(t)} = \frac{\phi'(t)v(t)- v'(t) \phi(t)}{v(t)^2} = \frac{\phi'(t)v(t) - C(t)\phi(t)v(t)}{v^2(t)} \le 0 
\end{equation*}
...
\begin{equation*}
    \frac{\phi(t)}{v(t)}\le \frac{\phi(a)}{v(a)} = \phi (a)
\end{equation*}
which by setting $a=0$ yields the result. 
\end{proof}
\end{theorem}
...
(2) \textbf{How entropy decay + Pinsker inequality imply bound on mixing time?}

\begin{definition}[Total variation distance]
Given two probability measures $\mu, \nu \in \mP(\X)$, the total variation (TV) distance between them is:
\begin{equation*}
    \lVert \mu-\nu\rVert_{TV}:= \max_{A\subseteq\X} |\mu(A)-\nu(A)|
    \end{equation*}
\end{definition}
...
Moreover, it stays in the following relation with the $L_1$ distance $\lVert \mu-\nu\rVert_1 := \sum_{\X}|\mu(x)-\nu(x)|$,
\begin{equation*}
    \lVert \mu-\nu \rVert_{TV} = \frac{1}{2} \lVert \mu-\nu \rVert_1
\end{equation*}

To see this, just notice that the probability mass in which $\mu\ge\nu$ must be equal to the probability mass in which the contrary happens, for the two to sum to 1. 

\begin{theorem}{Pinsker's inequality}
    Let $\mu$ and $\nu$ be two probability distributions over $\X$. Then
    \begin{equation*}
        \lVert\mu-\nu\rVert_{TV}^2 \le \frac{1}{2} D_{KL}(\mu||\nu)
    \end{equation*}
    
    \begin{proof}
        By what said above, we can equivalently prove that
        \begin{equation*}
            \frac{1}{2} \lVert\mu-\nu\rVert^2_1 \le D_{KL}(\mu||\nu)
        \end{equation*}

        Now, let $A= \{\mu(x)\ge\nu(x)\}$ and define the two Bernoulli random variables
        \begin{align*}
            &\mu_A := \begin{cases}
                1 & {\rm w.p}. \sum_{x\in A} \mu(x)\\
                0 & {\rm w.p}. \sum_{x \notin A} \mu(x)\\
            \end{cases}\\
            &\nu_A := \begin{cases}
                1 & {\rm w.p}. \sum_{x\in A} \nu(x)\\
                0 & {\rm w.p}. \sum_{x\notin A} \nu(x)\\
            \end{cases}
        \end{align*}
        Consider the following function
        \begin{equation*}
            f(\mu_A,\nu_A) := D_{KL}(\mu_A||\nu_A) - \frac{1}{2}{\lVert \mu_A-\nu_A\rVert_1}^2
        \end{equation*}
        Let us define $p:=\sum_{x\in A} \mu(x)$ and $q=\sum_{x\in A} \nu(x)$ to study $f$. 
        Clearly, if $p=q$ then $f=0$. Assuming w.l.o.g. $p\ge q$, we have that $f$ decreases with $q$: this is verifiable by making $f$ explicit and studying the derivative, seeing that the term in $D_{KL}$ (=$-\frac{1}{q(1-q)}$) leads the derivative over the term in the 1-norm (=4).  

        Then, we have that the function is decreasing in $q$ and =0 if $p=q$. Hence for $p\ge q$ we have $f\ge 0$ hence 
        \begin{equation*}
            D_{KL}(\mu_A || \nu_A) \ge \frac{1}{2}\lVert \mu_A - \nu_A \rVert^2_1
        \end{equation*}
        We have 
        \begin{align*}
            &D_{KL}(\mu || \nu) \ge D_{KL}(\mu_A || \nu_A) \\
            &\lVert \mu_A - \nu_A \rVert_1 = \lVert \mu - \nu \rVert_1
        \end{align*}
        ...
        the second one is easily observable by computing the 1-norm splitting the summation into $x\in A$ and $x \notin A$.
        Putting together the three inequalities yields the result:
        \begin{equation*}
            D_{KL}(\mu || \nu) \ge D_{KL}(\mu_A || \nu_A) \ge \frac{1}{2}{\lVert \mu - \nu \rVert_1}^2 = 2 \lVert \mu - \nu \rVert^2_{TV}
        \end{equation*}
    \end{proof}
\end{theorem}

For our aims it makes sense to consider $\nu =\pi$ where $\pi$ is the unifrom measure, and Pinsker's inequality becomes
\begin{equation}\tag{Pinsker}\label{eq: Pinsker}
    \lVert\mu-\pi\rVert_{TV}^2 \le \frac{1}{2} \mathcal{H}(\mu) - \log p_\pi
\end{equation} 
where $p_\pi$ is the mass assigned by the uniform distribution on each $x$. 
In our specific case, we look at the entropy of probability \textit{densities}, and the denisty of mass assigned by the uniform distribution is 1 everywhere, so the log term cancels. 

Recall also the entropy decay result
\begin{equation}\tag{Entropy decay}\label{eq: decay}
    \mathcal{H}(P_t \rho) \le e^{-2\lambda t}\mathcal{H}(\rho)
\end{equation}

Next we show how (\ref{eq: Pinsker}) and (\ref{eq: decay}) combine to obtain inequality (\ref{eq: mainresult}), our main result.

\begin{proof}[Proof of theorem (\ref{thm: result})]
    First, we use a result that comes from the old paper, sepcifically from the evolutional variational inequality:
\begin{equation*}
    \mathcal{H}(P_t \rho) \le \frac{\W(\rho, 1)}{4t}
\end{equation*}
This allows us to bound the entropy, $\mathcal{H}(P_t\rho) \le 2 $ for 
\begin{equation*}\tag{Bound \#1}
    t \ge \frac{D^2}{8}
\end{equation*}

Setting $t_0 = \frac{D^2}{8}$, we apply the entropy decay inequality considering this initial time.
Also, we consider the evolution of the chain in terms of probability \textit{measures}, accoridng to the dual semigroup $P_t^*$, to study convergence to the invariant measure $\pi$.

\begin{equation*}
    \lVert P_t^* \delta_x - \pi\rVert_{TV} \le \sqrt{\frac{1}{2} \mathcal{H}(P_t^* \delta_x)} \le \sqrt{\frac{1}{2} e^{-2\lambda t} \mathcal{H}(P_{t_0}^* \delta_x)} \le \sqrt{\frac{1}{2} e^{-2\lambda t} 2} = e^{\lambda t}
\end{equation*}

Then, we study for which $t$ we get $\varepsilon$-close to the invariant measure:
\begin{equation*}
    e^{\lambda t} \le \varepsilon; \quad -\lambda t \le \log \varepsilon;  \quad t \ge \frac{\log \varepsilon}{\lambda}
\end{equation*}

Substituting our constant $\lambda=\frac{c}{D^2}$ for $\Ric \ge 0$, we get the second bound
\begin{equation}\tag{Bound \#2}
    t \ge D^2\frac{\log\varepsilon}{c}
\end{equation}

Then, we sum the two bounds to obtain our resulting bound:
\begin{equation*}
    t \ge D^2 \bigg(\frac{1}{8} - \frac{\log\varepsilon}{c}\bigg) 
\end{equation*}
As $\tau_{\rm mix}$ is defined as the infimum of such times $t$, we get an upper bound on the mixing time as in (\ref{eq: mainresult}).
