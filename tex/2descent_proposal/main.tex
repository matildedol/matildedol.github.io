...
\definecolor{bluette}{RGB}{0, 123, 255} % bright bluish tone
\definecolor{darkorange}{RGB}{255, 140, 0}
\definecolor{pink}{RGB}{255, 130, 190}
% \theoremstyle{plain}


\title{On a mathematical explanation of double descent in stochastic gradient descent}
...
\begin{abstract}
    The phenomenon of double descent in stochastic gradient descent (SGD) explains why over-parameterized models 
    outperform under-parameterized ones, challenging the classical bias-variance tradeoff. Although it is well documented empirically, 
    a precise mathematical explanation is missing. Providing one could lead to a deeper 
    understanding of SGD and neural network training, contributing to the mathematical foundations of 
    machine learning.
    In this talk, I present my current work investigating the mathematical origins of double descent.
    I begin by discussing existing hypotheses and empirical results. I then identify the most promising research directions, including a 
    formal explanation of why SGD selects smoother interpolating solutions 
    and a characterization of the notion of smoothness it implicitly optimizes. I conclude by outlining the main technical 
    obstacles and possible strategies to address them.
\end{abstract}
\maketitle

The problem in question is {\bf finding the roots of the \textit{double descent} phenomenon}, concerning the stochastic gradient descent algorithm (SGD) for neural networks. 
\paragraph{Background} The double descent curve was first formalized by~\cite{belkin}, and it reconciles the theoretical result of the bias-variance tradeoff, which implies that capacity below the overfitting threshold is preferred, with the empirical findings that large, over-parametrized models perform well. 
...
This allows us to understand more in depth what is happening and to have a universal formalization of the properties and dynamics of the algorithm. While this field is highly practice-based, a theoretical framework that supports empirical results boosts the development of AI and further discoveries in this field, especially regarding performance and learning dynamics of neural networks.
...
By analyzing the update rule and the loss landscape mathematically, it should be possible to rigorously state what is happening after the overfitting threshold. There are some hypotheses, like the one proposed by~\cite{prince}, which compares the algorithm used in practice with its continuous analog, i.e. SGD with an infinitesimal step size. By doing so, we get that descending the gradient of the loss $\ell$ with a discrete step size $\alpha$ is equivalent to performing continuous gradient descent on the modified loss function.

\begin{proposition}
For a step size $\alpha\in\R$, $t\in \R$
\[
\theta_{t+\alpha}=\theta_t - \alpha \nabla\ell(\theta)\quad \text{and }\quad \frac{\mathrm{d} \theta}{\de t} = -\nabla\ell(\theta)
\]

Up to order $\alpha$, SGD corresponds to gradient flow of:
\begin{equation*}
...
\begin{proof}
    ...
    \end{equation*}
for $\tau \in \N$, $\alpha$ step size.
\\
Let $t=\alpha\tau \in \R$, and consider the finite-difference
\[
\theta_{t+1} - \theta_t = -\alpha\nabla\ell(\theta).
\]
Now, taking the limit we obtain the continuous-time update rule
\begin{align*}
&\lim_{\alpha\to0} \frac{\theta_{t+1} - \theta_t}{\alpha} = \frac{\de \theta}{\de t}\\
\end{align*}
\begin{equation}
    \frac{\de \theta}{\de t}= -\nabla\ell(\theta).
\end{equation}
...
\]
we want to find the correction  such that the two trajectories converge to the same parameter $\theta$.
...
&= \theta_0 - \alpha \nabla\ell(\theta) + \alpha^2 g(\theta) + \frac{1}{2}\alpha^2 \frac{\de}{\de t} \bigg(-\nabla\ell(\theta) +{\color{benred}\alpha g(\theta)}\bigg)\\
&\approx \theta_0 - \alpha \nabla\ell(\theta) + \alpha^2 \bigg(g(\theta) + \frac{1}{2}\bigg(-\nabla^2\ell(\theta)\frac{\de\theta}{\de t}\bigg)\bigg)\\
& = \theta_0 - \alpha \nabla\ell(\theta) + \alpha^2 \bigg(g(\theta) + \frac{1}{2}\bigg(\nabla^2\ell(\theta)\bigg(\nabla\ell(\theta) + {\color{benred} \alpha g(\theta)}\bigg)\bigg)\bigg)
\end{align*}

where the terms in red are of 3rd order in $\alpha$ and disregarded.

Then, we impose $\theta(t) = \theta_{\tau+1}$, obtaining
\begin{align*}
    & g(\theta) = -\frac{1}{2}\nabla^2\ell(\theta) \nabla\ell(\theta) ;\\
\end{align*}
Hence, the following continuous time update rule converges to the same value as the discrete time one:
\begin{equation}
...
This motivates the bias for less steep directions in the loss landscape. 
This result, however, requires to understand the connection between the gradient of the loss and a smoother learned model 
and it can be enriched with additional explanations and intuition.
