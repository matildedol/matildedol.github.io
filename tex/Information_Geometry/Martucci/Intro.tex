...
means of modern geometry'' \cite{amari2016information}, ``the field that studies the geometry of decision making'' \cite{nielsen2020elementary}, and ``the differential geometric treatment of statistical models'' \cite{ay2017information}.
...
The key geometric object is the so-called \textit{statistical manifold}, and it is formalized as a triple $$(\mathcal M, g, T),$$ where $\mathcal M$ is a smooth manifold, usually finite-dimensional, and $g$ is the so-called \textit{Fisher-Rao metric} which, in coordinates, we can just think of as a matrix. The pair $(\mathcal M, g)$ is the standard framework in Riemannian geometry. The additional object, $T$, is the \textit{Amari-Chenstov tensor} and is slightly more complicated than a metric: it takes as input three vectors and outputs one number, while the metric takes as input of two vectors.

There is an equivalent phrasing of this framework. We can use the tensor $T$ and the metric $g$ to build an object which is called a \textit{connection}, indicated as $\nabla$. We build $\nabla$ starting from $g$ and $T$, and then we find its dual connection $\nabla^*$, which is a well-defined connection. We can check duality by verifying that taking the dual is an involution, i.e. $$\nabla^{**}=\nabla$$ always holds. When $\nabla$ is flat, this duality is exactly the well-known Legendre duality from convex analysis. 

In Riemannian geometry, given a metric, we can always find a special type of connection, the Levi-Civita connection $\nabla^{LC}$, which induces the parallel transport, the geodesics, and so on ... and the study of the Levi-Civita connection is fundamental to Riemannian geometry. 
In our framework, the equality $$\nabla=\nabla^*$$ holds if and only if the connection we started with is exactly the Levi-Civita connection associated with $g$, $\nabla=\nabla^{LC}$. In this case, $\nabla$ is self-adjoint. In general, $\nabla$ is not Levi-Civita. 
\\

The natural question is: why do we care about this very abstract structure? The motivation comes from statistics. The natural metric to put on our manifold and the natural three-tensorial Amari Chenstov tensor are universal objects induced by \textit{Fisher information}. Recall the definition of Fisher information.
% \begin{definition}[Fisher information]
%     Given a probability density $p_\theta$ over $\Omega$, $\theta \in \Theta$, the Fisher information is defined as 
%     \[
%     FI_\theta = \int_\Omega \partial^2 
%     \]
% \end{definition}

Let's consider the standard framework in parametric statistics: pick a sample space $\Omega$, with a sigma algebra $\mathscr F$, then the pair $(\Omega, \mathscr F)$ is our \textit{sample} or \textit{state} \textit{space}. We consider a subset $\mathcal M\subset \mathcal P(\Omega)$ of the space of probability measures over $\Omega$, with some substructure. We assume these subsets are very regular, in particular they are smooth finite-dimensional submanifolds, which we can think of a sphere or embedded in the Euclidean space of dimension $n$. We use coordinates to parametrize locally the probability measures over $\mathcal M$ with parameters $\theta \in \Theta$, where $\Theta \subset \mathbb{R}^n$is a nice subset, such as an open set. Finally, we have the mapping $$\Theta\ni \theta\longmapsto P_\theta\in \mathcal M$$ that obtains probability measures that live on our $\mathcal M$. We ask for a few good properties of this parameterization: we will assume that this map is smooth with respect to the $\theta$ variable, but, for now, let's only assume that this is injective, so that we can look at the inverse. In many applications, we are able to parameterize the whole manifold with only one \textit{chart}.

The main conceptual point is that we can distinguish two different levels in this framework:
\begin{itemize}
    \item[-] the manifold of probability measures $M$ is the intrinsic object and it is given, it constitutes the \textit{abstract level};
...
\paragraph{\bf Obtaining tensors} Now, we need to find the metric $g$ and the tensor $T$. Actually, $g$ comes directly from the Fisher information and $T$ is a derived measure. We will see that this is essentially the only reasonable choice from the point of view of statistics. 
Let's start by writing down the tensor objects $g$ and $T$ informally. We assume that, for every $\theta$, our probability $P_\theta$ is absolutely continuous with respect to a certain measure $\mu$, i.e. $$P_\theta\ll\mu\in \mathcal P(\Omega)$$ meaning that there is a measure $\mu$ dominating all the probability measures $P_\theta$, or at least locally. Therefore, each probability measure has a density $p_\theta \coloneq \frac{\de P_\theta}{\de \mu}$, hence $$P_\theta=p_\theta\mu$$ and we can work with densities from now on.

Now, we define $g$, which is a metric hence it has two indices: \begin{align*}g_{ij}(\theta)&\coloneq\int_\Omega\frac{\partial}{\partial \theta_i}\log p_\theta(\omega)\frac{\partial}{\partial \theta_j}\log p_\theta(\omega)\, p_\theta(\omega)\, \text d\mu(\omega)\\
&=\int_\Omega\frac{\partial}{\partial \theta_i}\log p_\theta(\omega)\frac{\partial}{\partial \theta_j}\log p_\theta(\omega)\, P_\theta(\omega)\\
&=\mathbb E_\theta\left[\frac{\partial}{\partial \theta_i}\log p_\theta(\omega)\frac{\partial}{\partial \theta_j}\log p_\theta(\omega)\right]\end{align*} 
where $\partial_i \log p_\theta$ is the {\it score}, the derivative of the log-likelihood with respect to $\theta_i$. Therefore, $g$ arises as the {\it covariance matrix\/} of the score (up to imposing that the score has 0 expectation). 
%, as $\theta\subset\mathbb R^n$, which has coordinates, so we can differentiate with respect to this variable
When $i=j$, $g$ is exactly the Fisher information (the variance of the score).\footnote{Let us try to clarify a potential confusion: the Fisher information is generally defined as the variance of the score, hence $g_{ii}$, but we usually consider also the Fisher information {\it matrix\/}, which is the whole covariance matrix and in that case coincides with $g$. In one dimension the two objects coincide.}

Now, given coordinates $\theta \in \Theta$, it can be checked that this quadratic form $g$ is symmetric and positive definite, i.e. it is a Riemannian metric, so it induces a scalar product, which depends on the point $\theta$ we are studying. Pick two vectors $v, w\in \mathbb R^n$ and think of them as tangent vectors at a given $\theta\in \Theta$, then the inner product between the two is going to be $$g_\theta(v, w)=\sum_{i, j}v_iw_jg_{ij}(\theta)$$ where we wrote the two vectors in the standard Euclidean coordinates. In the current frame, we should think of abstract manifolds and abstract tangent spaces attached to these manifolds, but when we write things in coordinates, we are just looking at open sets of an $\mathbb R^n$, and these concepts kind of collapse into basic concepts of linear algebra. In the abstract picture, $\mathcal M$ is a manifold of measures, and $g$ will always take the Fisher-like form.
...
\begin{example}
    ...
    0 && 2/\sigma^2\end{pmatrix}$$ with constant curvature equal to $-1$. So, Gaussians with this natural metric are like a model for the hyperbolic geometry. 

Moreover, this metric blows up as $\sigma\uparrow+\infty$ that geometrically means that the boundary of this half space is being sent to infinity. Namely, we should think of it as a two-dimensional plane with a metric with constant negative curvature, and not as a half space. However, the constant -1 curvature is the geometry induced in terms of the Levi-Civita connection associated to $g$, but this is not the framework of information geometry, where we should ask about the geometry of the already cited connection $\nabla$, which we shall define. The correct connection for our framework is something else and depends also on the Amari tensor.

The fact that the mean has no appearance in the metric is justified by the fact that the half-plane is hyperbolic in the $\sigma$-direction and cylindrical in $\mu$, so that shifting $\mu$ is sort of an isometry in terms of the form of the bell-shaped Gaussian.\end{example}
\begin{example}
